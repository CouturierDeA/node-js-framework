...
export class ServerEvents extends EventEmitter {
    ...
    public async init(
        ...
        console.info(
            `inited; connections  ${ServerEvents.connections}; stream id ${response.id}`
        );
        ...
        const dataListener = (event: { eventName: string; data: any }) => {
        ...
        response.on('close', () => {
            ...
            onClose?.(this);
            console.info(
                `destroyed; connections  ${ServerEvents.connections}; stream id ${response.id}`
            );
        });

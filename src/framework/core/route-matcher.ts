...
export const routeMatcherFnCurly = (
    route: string | RegExp,
    url: string,
    routeItem?: IRoute
): RequestParams => {
    ...
    const query = matched
        ? parseQuery(queryString ? `?${queryString}` : undefined)
        : {};
...
export function routeMatcher<T>(
    url: string,
    routes: IRoute[],
    method: string
): {
    ...
    const middleware = routes
        ...
        .map((route) => {
            ...
            const methodMatched =
                routeMethod.toLowerCase() === method.toLowerCase() ||
                routeMethod === '*';
            if (!methodMatched) {
                return undefined;
            }
    ...
    let executors = routes
        ...
        .map((route) => {
            ...
            const methodMatched =
                routeMethod.toLowerCase() === method.toLowerCase();
            const result =
                methodMatched && routeMatcherFnCurly(routePath, url, route);
            if (result?.matched) {
                Object.assign(match, result);
            }
            return result?.matched && (route.executor as unknown as T);
    ...
    return {
        executors: [...middleware, ...executors],
        requestParams,
    };
}
...
export function parseParamsCurly(route: string, url1: string) {
    ...
    if (params?.length) {
        ...
        return params?.reduce(
            (acc: { res: any; add: Function }, v: any, index: number) =>
                acc?.add(v, index),
            {
                res: {},

...
function applyMapping(
    ...
    defineMappingMeta(target.constructor, propertyKey, options);
}
...
export function PathVariable(key?: string) {
    return function (target: any, propertyKey: string, parameterIndex: number) {
        ...
        const keyName: string =
            key ||
            Object.values(getParamNames(target[propertyKey]))[parameterIndex] ||
            'path';
        addArgInjector(
            target,
            propertyKey,
...
export function QueryParams(options?: {
    key?: string;
    serializer?: StringConstructor | NumberConstructor;
}) {
    return function (target: any, propertyKey: string, parameterIndex: number) {
        const { key, serializer } = options || {};
        const defaultSerializer = serializer || String;
        const serialize = Reflect.getMetadata(
            'design:paramtypes',
            target,
            propertyKey
        )[parameterIndex];
            ...
            new ArgumentsInjector<Sandbox>(async (sandBox) => {
                ...
                if (serialize.name !== 'Array') {
                    throw ApiException.internal(
                        `QueryParams must be of array type, got ${serialize.name}`
                    );
                }
                return qs
                    ? new serialize(...qs?.map((q) => defaultSerializer(q)))
                    : qs;
            })
        );
    };
...
function addArgInjector<T>(
    target: any,
    propertyKey: string,
    parameterIndex: number,
    argInjector: ArgumentsInjector<T>
) {
    defineArgumentInjections(target, propertyKey, parameterIndex, argInjector);
    defineArgumentInjectionsOnConstructor(
        target,
        propertyKey,
        parameterIndex,
...
export function setupExecutorArguments<T>(
    target: any,
    propertyKey: string,
    executor: (...args: any[]) => Promise<any>,
    args: ArgsInjectionOnConstructor<T>['args']
): Function {
    ...
    return async (...args: any[]) => {
        ...
        const injectionResults = await Promise.all(
            injectionKeys.map((index) =>
                argumentInjections[index].inject(sandbox)
            )
        );
        const newArgs = [...args];
        injectionKeys.forEach(
            (injectionArgumentKey, injectionArgumentIndex) => {
                newArgs[injectionArgumentKey] =
                    injectionResults[injectionArgumentIndex];
            }
        );
...
function getRequestBody<T extends object>(
    serializer: (stream: string) => T = qsToJson
) {
    return function (target: any, propertyKey: string, parameterIndex: number) {
        addArgInjector(
            target,
...
export class ArgumentsInjector<T> {
    constructor(executor: (sandBox: T) => Promise<any>) {

...
export class App {
    ...
    public onRequest = (req: Http2ServerRequest, res: Http2ServerResponse) => {
    ...
    private spawnRouteController<T extends { new (...args: any[]): {} }>(
        controller: T
    ) {
        ...
        const routeCtrl: RouteCtrl<T> = routeCreator.createRouteController(
            controller as any
        );
    ...
    private spawnControllers() {
        this.controllers.forEach((controller) =>
            this.spawnRouteController(controller)
        );
    }
}

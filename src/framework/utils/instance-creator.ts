...
export function instanceCreator<T extends { new (...args: any[]): {} }>(
    ...
    const argInjections = getArgumentInjectionsFromConstructor(
        constructor
    ).filter((inj, index, self) => {
        return (
            self.findIndex((s) => s.propertyKey === inj.propertyKey) === index
        );
    });
    ...
    const instantiate = async (di: DiContainer) => {
        ...
        const createdHook: Function = Reflect.getOwnMetadata(
            CREATED_HOOK_KEY,
